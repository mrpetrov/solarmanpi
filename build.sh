...
then
    ...
    echo "$(tput setaf 1)$(tput setab 7)ERROR: No solard.c found in $(tput smso)`pwd`$(tput smso)!$(tput sgr 0)"
    exit 2
fi
...
if ! gcc -D_FORTIFY_SOURCE=2 -DSOLARDVERSION=\"`git describe --tag`\" -Wall -Wno-unused-result -O3 -o solard solard.c; then
	...
	echo "$(tput setaf 2)Successfully compiled solard version $(tput smso)`git describe --tag`$(tput smso) at: $(tput setaf 4)`date`$(tput sgr 0)"
fi
